...
public class GuiRadiobutton extends Gui {
	...
	public void draw() {
		...
		drawRect(xPos + tw + 5 , yPos, xPos + 5 + tw + width, yPos + height, Color.gray.getRGB());
		if (checked) {
			drawRect(xPos + 6 + tw, yPos + 1, xPos + 5 + tw + 1 + width - 2, yPos + 1 + height - 2,
					Color.green.getRGB());
		} else {
			drawRect(xPos + 6 + tw, yPos + 1, xPos + 5 + tw + 1 + width - 2, yPos + 1 + height - 2,
					Color.black.getRGB());
	...
	public boolean isMouseHoovering(int x, int y) {
		int tw = fontrenderObj.getStringWidth(text);
		return x > xPos+5 && x < xPos + tw + 5 + width && y > yPos && y < yPos + height;
	}
