...
public class GuiModInit extends GuiScreen {
	...
	public void drawScreen(int x, int y, float f) {
		...
		RenderUtils.renderEntityAtPos(Minecraft.getMinecraft().thePlayer, width - 100, height - 50, 90, 0, 0, 6.5f,
				6.5f);
		...
		if (b.id == 0) {
			if (!this.namefield.getText().isEmpty() && !this.surnamefield.getText().isEmpty()
				...
				if (RLMPlayerProps.get(Minecraft.getMinecraft().thePlayer) != null) {
					...
					props.name = namefield.getText();
					props.surname = surnamefield.getText();
					props.gender = EnumGender.getFromString(genderGroup.getSelectedbutton().getText());
					System.out.println("Set the name to: " + namefield.getText() + " and the Surname to: "
							+ surnamefield.getText() + "!");
					RealLifeMod.network.sendToServer(new SetPropertiesPackage(
							Minecraft.getMinecraft().thePlayer.getEntityId(), this.surnamefield.getText(),
							this.genderGroup.getSelectedbutton().getText(), this.namefield.getText()));
